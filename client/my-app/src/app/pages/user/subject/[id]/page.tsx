...
export default function Page({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchExams = async () => {
      try {
        ...
        const subjectIdsInCourse = subjects.map(subject => subject.id);

        const filteredExams = response.data.filter(exam => {
          const isSubjectInCourse = subjectIdsInCourse.includes(exam.idSubject.toString());
          return (
            isSubjectInCourse &&
  ...
  }, [selectedSubject, selectedLevel, subjects]); // Add 'subjects' to dependencies

  const handleSubjectClick = (subjectId: string) => {
    setSelectedSubject(parseInt(subjectId, 10));
  };
